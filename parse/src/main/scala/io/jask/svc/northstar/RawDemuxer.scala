...
class RawDemuxer(consumer: KafkaConsumer[UUID, Array[Byte]], config: Config) extends Runnable {
  ...
  private[this] lazy val log = LoggerFactory
    .getLogger(classOf[RawDemuxer])
  private[this] var outputs: mutable.Map[String, FileOutputStream] =
    mutable.Map()

  /** Rebuild outputs map on SIGHUP */
  Signal.handle(new Signal("HUP"), _ => buildOutputsMap())
            ...
            cursor.downField("msg").downField("tag").as[String] match {
              ...
              case Right(tag) =>
                ...
                if (outputs.contains(tag)) {
                  cursor.downField("msg").downField("content").as[String] match {
                    case Left(_) => log.error("No content present.")
                    case Right(content) =>
                      try {
